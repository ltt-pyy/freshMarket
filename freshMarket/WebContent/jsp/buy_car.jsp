...
%>
	...
	Iterator<Cart> iter = all.iterator() ;
		...
		while(iter.hasNext()){
			Cart cart=iter.next();
			String freshId=cart.getFreshId();
						...
						<table class="table table-bordered table-striped"> 
							...
							<tr>
							    ...
							    <td>
								    ...
								    <p>
									   金额：￥<%=fresh.getNew_price()>0?fresh.getNew_price():fresh.getPrice() %>
							    ...
							    <td>
							    	<p>
									订单金额：￥<%=(fresh.getNew_price()>0?fresh.getNew_price():fresh.getPrice())*cart.getCounts() %>
								    </p>
							    </td>
							    ...
							    <td>
							        <form action="FreshInfoServlet">
							        <input type="hidden" name="id" value=<%=request.getSession().getAttribute("id")%>>
							        <input type="hidden" name="freshId" value=<%=fresh.getFreshId()%>>
