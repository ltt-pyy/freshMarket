...
public class AddressInfoDAOImpl implements IAddressInfoDAO {
	...
	public void doUpdateCount(Integer id) throws Exception {
		String sql = "UPDATE addressinfo SET count=count+1 WHERE addId=?";
	...
	public List<AddressInfo> findAll(Set<Integer> key) throws Exception {
		...
		sql
				.append("SELECT addId,userId,addname,addr,phone FROM addressinfo WHERE addId IN (");
		int count = 0;
		...
		while (rs.next()) {
			...
			addr.setaddId(rs.getString(1));
			addr.setuserId(rs.getString(2));
	...
	public boolean doCreate(AddressInfo vo) throws Exception {
		...
		String sql = "INSERT INTO addressinfo(addId,userId,addname,addr,phone)"
				+ " VALUES (?,?,?,?,?) ";
		this.pstmt = this.conn.prepareStatement(sql);
		this.pstmt.setString(1, vo.getaddId());
	...
	public boolean doRemove(String id) throws Exception {
		boolean flag = false;
		String sql = "DELETE FROM addressinfo WHERE addId=? ";
		this.pstmt = this.conn.prepareStatement(sql);
	...
	public boolean doUpdate(AddressInfo vo) throws Exception {
		boolean flag = false;
		String sql = "UPDATE addressinfo SET userId=?,addname=?,addr=?,phone=?  WHERE addId=? ";
		this.pstmt = this.conn.prepareStatement(sql);
		this.pstmt.setString(5, vo.getaddId());
	...
	public List<AddressInfo> findAll(String keyWord) throws Exception {
		List<AddressInfo> all = new ArrayList<AddressInfo>();
		String sql = "SELECT addId,userId,addname,addr,phone"
				+ " FROM addressinfo WHERE userId LIKE ? OR addname LIKE ?  ";
		this.pstmt = this.conn.prepareStatement(sql);
	...
	public long getAllCount(String keyWord) throws Exception {
		long count = 0;
		String sql = "SELECT COUNT(orderno) FROM fresh_order "
	...
	public AddressInfo findById(String id) throws Exception {
		...
		String sql = "SELECT addId,userId,addname,addr,phone  FROM addressinfo WHERE addId=?";
		this.pstmt = this.conn.prepareStatement(sql);
		this.pstmt.setString(1, id);
		ResultSet rs = this.pstmt.executeQuery();
		if (rs.next()) {
